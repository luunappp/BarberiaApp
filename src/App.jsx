...
const sb = {
  ...
  async getAll() {
    ...
    if (!res.ok) { console.error("Supabase getAll error", res.status, await res.text()); return []; }
    return res.json();
  },
  async insert(data) {
    ...
    if (!res.ok) { console.error("Supabase insert error", res.status, await res.text()); return null; }
    return res.json();
  },
  async remove(id) {
    const res = await fetch(SUPABASE_URL + "/rest/v1/reservas?id=eq." + id, { method: "DELETE", headers: this.headers });
    if (!res.ok) { console.error("Supabase delete error", res.status, await res.text()); }
  }
};
...
// Turnos comunes para todos los días
const COMUNES = ["09:00", "10:50", "12:20", "12:40"];
...
const HORARIOS_POR_DIA = {
  ...
  4: ["09:00", "10:50", "11:20", "11:50", "12:20", "12:40", "13:00", "13:20"],  // Jueves (30 min entre turnos nuevos)
  5: ["09:00", "10:50", "11:20", "11:50", "12:20", "12:40", "13:00", "13:20"],  // Viernes (30 min entre turnos nuevos)
  6: []
};
...
function Calendario(props) {
  ...
  const mesV = props.mesV; const setMesV = props.setMesV;
  const anioV = props.anioV; const setAnioV = props.setAnioV;
  const setDiaSelec = props.setDiaSelec; const setVista = props.setVista;
  ...
  return (
    <div style={{ animation:"fadeUp .35s ease" }}>
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:16 }}>
        <button style={st.navBtn} disabled={!puedeAtras}
          onClick={function() { if(mesV===0){setMesV(11);setAnioV(function(a){return a-1;});}else{setMesV(function(m){return m-1;});} }}>‹</button>
        <span style={{ fontFamily:"'Playfair Display',serif", fontSize:20, fontWeight:700 }}>{MESES[mesV]} {anioV}</span>
        <button style={st.navBtn}
          onClick={function() { if(mesV===11){setMesV(0);setAnioV(function(a){return a+1;});}else{setMesV(function(m){return m+1;});} }}>›</button>
      </div>
      <div style={st.grid7}>
        {DIAS_CORTO.map(function(d) {
          return <div key={d} style={{ textAlign:"center", fontSize:11, color:C.textD, fontWeight:600, padding:"4px 0" }}>{d}</div>;
        })}
      </div>
      <div style={st.grid7}>
        {celdas.map(function(dia, i) {
          ...
          return (
            <button key={dia} disabled={deshabilitado}
              onClick={function() { setDiaSelec({ dia:dia, fecha:fecha, label:dia+" de "+MESES[mesV], diaSem:diaSem }); setVista("horas"); }}
              style={Object.assign({}, st.diaBtn, esHoy?st.diaBtnHoy:{}, deshabilitado?{opacity:0.25,cursor:"not-allowed"}:{})}>
              <span>{dia}</span>
...
function Horarios(props) {
  ...
  return (
    <div style={{ animation:"fadeUp .3s ease" }}>
      <button style={st.back} onClick={function() { setVista("cal"); }}>← {diaSelec ? diaSelec.label : ""}</button>
...
function Formulario(props) {
  ...
  return (
    <div style={{ animation:"fadeUp .3s ease" }}>
      <button style={st.back} onClick={function() { setVista("horas"); }}>← {HORARIOS_LABEL[horaSelec]}</button>
      <p style={st.subTitulo}>Completa tu reserva</p>
      <div style={st.pillResumen}>📅 {diaSelec ? diaSelec.label : ""} &nbsp;·&nbsp; 🕐 {HORARIOS_LABEL[horaSelec]}</div>
      <p style={st.labelField}>Servicio</p>
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8, marginBottom:18 }}>
        {SERVICIOS.map(function(s) {
          return (
            <button key={s.id} onClick={function() { setSrvSelec(s.id); }}
      ...
      <input value={nombre} onChange={function(e) { setNombre(e.target.value); }}
        placeholder="Escribe tu nombre completo" style={st.input} />
      <p style={st.labelField}>Sección</p>
      <div style={{ display:"flex", gap:8, flexWrap:"wrap", marginBottom:24 }}>
        {SECCIONES.map(function(s) {
          return (
            <button key={s} onClick={function() { setSeccion(s); }}
              style={Object.assign({}, st.secBtn, seccion === s ? st.secBtnActivo : {})}>{s}</button>
          );
        })}
      </div>
      <button disabled={!listo}
        onClick={function() { onConfirmar(nombre.trim(), seccion, SERVICIOS.find(function(x) { return x.id === srvSelec; })); }}
  ...
  return (
    <div style={{ animation:"popIn .4s cubic-bezier(.175,.885,.32,1.275)", textAlign:"center", padding:"10px 0" }}>
      ...
      <div style={{ background:"rgba(245,197,24,.08)", border:"2px dashed "+C.gold, borderRadius:16, padding:"16px", marginBottom:20, animation:"pulse 2s infinite" }}>
        ...
        <p style={{ fontFamily:"'Playfair Display',serif", fontSize:34, fontWeight:900, color:C.gold, letterSpacing:4 }}>{ultima ? ultima.codigo : ""}</p>
      ...
      <div style={st.resumenCard}>
        ...
        <div style={{ display:"flex", justifyContent:"space-between", paddingTop:12, marginTop:4 }}>
          ...
          <span style={{ color:C.gold, fontWeight:700, fontSize:20 }}>{formatPrecio(ultima ? ultima.servicio_precio : 0)}</span>
        </div>
      </div>
...
function Cancelacion(props) {
  ...
  return (
    <div style={{ animation:"fadeUp .3s ease" }}>
      ...
      <p style={Object.assign({}, st.subTitulo, { marginBottom:20 })}>Ingresa el código único que recibiste al reservar</p>
      {!encontrada ? (
        <>
        ...
        <div style={{ animation:"fadeUp .3s ease" }}>
          ...
          <button onClick={function() { setEncontrada(null); setCodigo(""); }} style={st.btnSecundario}>← Volver</button>
        </div>
      )}
    </div>
  );
...
function LoginAdmin(props) {
  ...
  function intentar() {
    ...
    else { setError(true); setShake(true); setTimeout(function() { setShake(false); }, 500); }
  }
  ...
  return (
    <div style={{ animation:"fadeUp .35s ease", display:"flex", flexDirection:"column", alignItems:"center", padding:"30px 0" }}>
      ...
      <div style={{ width:"100%", animation: shake ? "shake .4s ease" : undefined }}>
        ...
        <input value={user} onChange={function(e) { setUser(e.target.value); setError(false); }}
          onKeyDown={function(e) { if (e.key==="Enter") intentar(); }} placeholder="Mario Sánchez"
          style={Object.assign({}, st.input, error ? { borderColor:C.red } : {})} />
        <p style={st.labelField}>Contraseña</p>
        <input type="password" value={pass} onChange={function(e) { setPass(e.target.value); setError(false); }}
          onKeyDown={function(e) { if (e.key==="Enter") intentar(); }} placeholder="••••••••••"
          style={Object.assign({}, st.input, error ? { borderColor:C.red } : {})} />
...
function TarjetaAdmin(props) {
  const r = props.r; const onEliminar = props.onEliminar;
  return (
...
function PanelAdmin(props) {
  const reservas = props.reservas; const onLogout = props.onLogout; const onEliminar = props.onEliminar;
  const [busq, setBusq] = useState("");
  const citasHoy = reservas.filter(function(r) { return r.fecha === hoyStr(); }).length;

  const agrupadas = {};
  reservas.forEach(function(r) { if (!agrupadas[r.fecha]) agrupadas[r.fecha] = []; agrupadas[r.fecha].push(r); });
  const grupos = Object.entries(agrupadas).sort(function(a,b) { return a[0].localeCompare(b[0]); });
  const filtradas = busq.trim() ? reservas.filter(function(r) { return r.nombre.toLowerCase().includes(busq.toLowerCase()); }) : null;
  ...
  return (
    <div style={{ animation:"fadeUp .3s ease" }}>
      ...
      <div style={{ display:"flex", gap:8, marginBottom:16 }}>
        <input value={busq} onChange={function(e) { setBusq(e.target.value); }} placeholder="Buscar por nombre..."
          style={Object.assign({}, st.input, { marginBottom:0, flex:1, fontSize:13, padding:"10px 12px" })} />
        {busq && <button onClick={function() { setBusq(""); }} style={{ background:C.cardB, border:"1px solid #2e3e2e", borderRadius:10, padding:"0 12px", color:C.textM, fontSize:13 }}>✕</button>}
      </div>
      {busq.trim() ? (
        filtradas.length === 0
          ? <p style={{ color:C.textD, textAlign:"center", padding:20 }}>No se encontró ninguna cita.</p>
          : filtradas.map(function(r) { return <TarjetaAdmin key={r.id} r={r} onEliminar={function() { onEliminar(r); }} />; })
      ) : grupos.length === 0
          ? <p style={{ color:C.textD, textAlign:"center", padding:30 }}>No hay citas agendadas.</p>
          : grupos.map(function(entry) {
              const fecha = entry[0]; const citas = entry[1];
              ...
              return (
                <div key={fecha} style={{ marginBottom:20 }}>
                  ...
                  {ordenadas.map(function(r) { return <TarjetaAdmin key={r.id} r={r} onEliminar={function() { onEliminar(r); }} />; })}
                </div>
              );
            })
      }
...
export default function App() {
  ...
  async function cargarReservas(esInicial) {
    try {
      ...
      for (let i = 0; i < pasadas.length; i++) { await sb.remove(pasadas[i].id); }
  ...
  useEffect(function() {
    cargarReservas(true);
    const intervalo = setInterval(function() { cargarReservas(false); }, 8000);
    return function() { clearInterval(intervalo); };
  }, []);

  function mostrarNotif(txt) { setNotif(txt); setTimeout(function() { setNotif(null); }, 3500); }

  async function handleConfirmar(nombre, seccion, srv) {
    setGuardando(true);
    const codigo = generarCodigo();
    const nueva = { nombre:nombre, seccion:seccion, fecha:diaSelec.fecha, hora:horaSelec,
      servicio_id:srv.id, servicio_label:srv.label, servicio_precio:srv.precio,
      servicio_icon:srv.icon, fecha_label:diaSelec.label, codigo:codigo };
  ...
  async function handleCancelar(r) { await sb.remove(r.id); await cargarReservas(false); mostrarNotif("🗑️ Cita de " + r.nombre + " cancelada"); }
  async function handleEliminar(r) { await sb.remove(r.id); await cargarReservas(false); mostrarNotif("🗑️ Cita de " + r.nombre + " eliminada"); }
  ...
  return (
    <>
      ...
      <div style={st.shell}>
        ...
        <div style={st.tabs}>
          {[{key:"cliente",label:"📅 Reservar"},{key:"cancelar",label:"🗑️ Cancelar"},{key:"barbero",label:"💈 Barbero"}].map(function(t) {
            return (
              <button key={t.key} onClick={function() { setTab(t.key); if(t.key==="cliente") setVista("cal"); }}
            ...
            <>
              {tab === "cliente" && (
                <>
                  ...
                  {vista === "ok" && <Confirmacion ultima={ultima} onNueva={function() { setVista("cal"); setDiaSelec(null); setHoraSelec(null); }} />}
                </>
              )}
